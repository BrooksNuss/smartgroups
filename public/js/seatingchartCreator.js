angular.module("mainApp").controller('seatingchartCreatorController', function($scope, $stateParams, $rootScope, studentByClassService, seatingchartService) {

	$scope.studentNames = [] ;
	$scope.startData = [];
	$scope.keyIndex = 0;

  //Begin seating chart creator initialization
	if($rootScope.currentChart){
		$scope.studentArr = studentByClassService.query({_id: $rootScope.currentClass._id}, function(res){
			res.forEach(function(student){
				$scope.studentNames.push({key: student.name});
			});
			if($rootScope.seatingReset){
	  			seatingchartService.get({_id: $rootScope.currentChart._id}, function(dbchart){
	  				console.log(dbchart);
	  				$scope.startData = dbchart.data;
	  				$scope.startData.forEach(function(dataItem){
	  					if(!dataItem.guests){
	  						dataItem.guests = {};
	  					}
	  					if(dataItem.category){
	  						if(dataItem.key>$scope.keyIndex)
	  							$scope.keyIndex=dataItem.key+1;
	  					}
	  				})
	  				init();
	  			})
  ...
  //Save current chart data to database
	$scope.updateChart = function(){
		$rootScope.currentChart.data = myDiagram.model.Ce;
		seatingchartService.update($rootScope.currentChart);
	}

  //Create new table/desk on seating chart
	$scope.addTable = function(type){
		myDiagram.model.Ce.push({"key":$scope.keyIndex, "category":type, "name":"", "guests":{}, "loc":"150 50"});
		myDiagram.model = new go.GraphLinksModel(myDiagram.model.Ce);
	}

	//SEATING CHARTS PLUGIN
  //The following code comes from the GoJS seating charts sample.
  //This itilizes the go.js file in this directory
	function init() {
    ...
    myDiagram.nodeTemplateMap.add("",  // default template, for people
      $(go.Node, "Auto",
        ...
        $(go.Shape, "Rectangle", { fill: "#b1eaab", stroke: null }),
        $(go.Panel, "Viewbox",
    ...
    function Seat(number, align, focus) {
      ...
      return $(go.Panel, "Spot",
               ...
               $(go.Shape, "Circle",
                 { name: "SEATSHAPE", desiredSize: new go.Size(40, 40), fill: "#abceea", stroke: "white", strokeWidth: 2 },
                 new go.Binding("fill")),
    ...
    myDiagram.nodeTemplateMap.add("TableR8",  // rectangular with 8 seats
      $(go.Node, "Spot", tableStyle(),
        $(go.Panel, "Spot",
          $(go.Shape, "Rectangle",
            { name: "TABLESHAPE", desiredSize: new go.Size(160, 80), fill: "#abceea", stroke: null },
    ...
    myDiagram.nodeTemplateMap.add("Desk",  // rectangular with 8 seats
      $(go.Node, "Spot", tableStyle(),
        $(go.Panel, "Spot",
          $(go.Shape, "Rectangle",
            { name: "TABLESHAPE", desiredSize: new go.Size(40, 40), fill: "#abceea", stroke: null },
            new go.Binding("desiredSize", "size", go.Size.parse).makeTwoWay(go.Size.stringify),
            new go.Binding("fill")),
          $(go.TextBlock, { editable: true, font: "bold 11pt Verdana, sans-serif" },
            new go.Binding("text", "name").makeTwoWay(),
    ...
    myDiagram.nodeTemplateMap.add("TableR3",  // rectangular with 3 seats in a line
      $(go.Node, "Spot", tableStyle(),
        $(go.Panel, "Spot",
          $(go.Shape, "Rectangle",
            { name: "TABLESHAPE", desiredSize: new go.Size(160, 60), fill: "#abceea", stroke: null },
            new go.Binding("desiredSize", "size", go.Size.parse).makeTwoWay(go.Size.stringify),
            new go.Binding("fill")),
          $(go.TextBlock, { editable: true, font: "bold 11pt Verdana, sans-serif" },
            new go.Binding("text", "name").makeTwoWay(),
    ...
    myDiagram.nodeTemplateMap.add("TableC8",  // circular with 8 seats
      $(go.Node, "Spot", tableStyle(),
        $(go.Panel, "Spot",
          $(go.Shape, "Circle",
            { name: "TABLESHAPE", desiredSize: new go.Size(120, 120), fill: "#abceea", stroke: null },
            new go.Binding("desiredSize", "size", go.Size.parse).makeTwoWay(go.Size.stringify),
            new go.Binding("fill")),
          $(go.TextBlock, { editable: true, font: "bold 11pt Verdana, sans-serif" },
            new go.Binding("text", "name").makeTwoWay(),
    ...
    if($scope.startData.length>0)
    	myDiagram.model = new go.GraphLinksModel($scope.startData);  // this sample does not make use of any links
    else
    ...
    for(var j=0;j<myDiagram.model.Ce.length;j++){
      for(var i=0;i<$scope.studentNames.length;i++){
        if(myDiagram.model.Ce[j].key===$scope.studentNames[i].key){
          $scope.studentNames.splice(i, 1);
          i--;
        }
      }
    }

    myGuests.model = new go.GraphLinksModel($scope.studentNames);
